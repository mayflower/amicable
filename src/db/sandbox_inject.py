...
def render_runtime_js() -> str:
    ...
    return (
        ...
        "  }\n"
        "  var parentOrigin='';\n"
        "  try{parentOrigin=decodeURIComponent(_param('amicableParentOrigin')||'');}catch(e){parentOrigin='';}\n"
        ...
        "  }\n"
        "  function _send(kind,message,stack,url,extra){\n"
        "    var now=Date.now();\n"
        "    if(now-lastSentAt<MIN_INTERVAL_MS) return;\n"
        "    var base=String(kind||'')+'|'+String(message||'')+'|'+String(stack||'')+'|'+String(url||'');\n"
        ...
        "        }\n"
        "      }\n"
        "      _send('window_error',msg,st,(ev&&ev.filename)||location.href,null);\n"
        "    }catch(e){}\n"
        "  },true);\n"
        ...
        "        msg=String(r);\n"
        "      }\n"
        "      _send('unhandled_rejection',msg,st,location.href,null);\n"
        "    }catch(e){}\n"
        "  });\n"
        ...
        "                  _send('graphql_error',String(m||'GraphQL error'),'',url,{first_error:first});\n"
        "                }\n"
        "              }catch(e){}\n"
        "            }).catch(function(){});\n"
        "          }\n"
