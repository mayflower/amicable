...
def test_runtime_probe_fails_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AMICABLE_RUNTIME_PROBE_ATTEMPTS", "3")
    monkeypatch.setenv("AMICABLE_RUNTIME_PROBE_BASE_DELAY_MS", "1")
    monkeypatch.setattr("src.agent_core.time.sleep", lambda _s: None)

    agent = Agent()
    backend = _FailingBackend()
    with pytest.raises(RuntimeError, match="runtime_unreachable"):
