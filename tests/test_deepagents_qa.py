...
class TestDeepAgentsQa(unittest.TestCase):
    def test_detects_commands_from_scripts(self):
        pkg = {"scripts": {"lint": "eslint .", "typecheck": "tsc -b", "build": "vite build"}}
        cmds = detect_qa_commands(pkg)
        self.assertEqual(cmds, ["npm run -s lint", "npm run -s typecheck", "npm run -s build"])
    ...
    def test_run_qa_fails_fast(self):
        ...
        passed, results = run_qa(b, ["npm run -s lint", "npm run -s build", "npm run -s typecheck"])
        self.assertFalse(passed)
        self.assertEqual([r.command for r in results], ["npm run -s lint", "npm run -s build"])
    ...
    def test_python_qa_commands(self):
        self.assertEqual(python_qa_commands(run_tests=False), ["python -m compileall -q .", "ruff check ."])
        self.assertEqual(
